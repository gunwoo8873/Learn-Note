...
// Note : 변수는 기본적으로 { } 내에서만 유효하며 if문 { } 안에서 선언된 변수도 if문 안에서만 사용 가능하다.
...
// Note : let 은 기본적으로 값의 업데이트가 불가능한 시스템으로 형성 되어 있다.
...
// Note : const 는 Immutable 같은 특징을 가지고 있지만 mut 와 같이 사용이 불가능 하다.
